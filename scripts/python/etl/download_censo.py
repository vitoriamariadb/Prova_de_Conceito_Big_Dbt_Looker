...
BASE_URL_CENSO = "https://download.inep.gov.br/dados_abertos/microdados_censo_escolar_{ano}.zip"
...
def download_censo_escolar(ano: int, extrair: bool = True) -> Path:
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    url = BASE_URL_CENSO.format(ano=ano)
    arquivo_zip = DATA_DIR / f"microdados_censo_escolar_{ano}.zip"
    pasta_destino = DATA_DIR / f"censo_escolar_{ano}"

    if pasta_destino.exists() and any(pasta_destino.iterdir()):
        print(f"Censo {ano} ja existe em: {pasta_destino}")
        return pasta_destino

    if not arquivo_zip.exists():
        sucesso = download_arquivo(url, arquivo_zip)
        if not sucesso:
            url_alternativa = f"https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/microdados/censo-escolar"
...
def listar_arquivos_censo(pasta: Path) -> list:
...
def main():
    print("=" * 70)
    print("DOWNLOAD MICRODADOS CENSO ESCOLAR - INEP")
    print("=" * 70)
    print()
    ...
    pasta = download_censo_escolar(ano)
    ...
    if pasta:
        ...
        arquivos = listar_arquivos_censo(pasta)
