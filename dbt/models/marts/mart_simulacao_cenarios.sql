...
SELECT
    ...
    ROUND(m.NOTA_MEDIA_NACIONAL, 2) AS NOTA_BASE,
    ROUND(c.AUMENTO_PERCENTUAL * 0.4, 2) AS IMPACTO_NOTA_ENEM_PONTOS,
    ROUND(m.NOTA_MEDIA_NACIONAL + (c.AUMENTO_PERCENTUAL * 0.4), 2) AS NOTA_PROJETADA,
    ROUND(c.AUMENTO_PERCENTUAL * 0.08, 2) AS REDUCAO_ABANDONO_PCT,
    ROUND(c.AUMENTO_PERCENTUAL * 0.03, 2) AS IMPACTO_IDEB_PONTOS,
    CASE
        WHEN c.AUMENTO_PERCENTUAL <= 10 THEN 'Baixo risco'
        WHEN c.AUMENTO_PERCENTUAL <= 20 THEN 'Risco moderado'
        ELSE 'Alto risco fiscal'
    END AS AVALIACAO_RISCO,
    CASE
        WHEN c.AUMENTO_PERCENTUAL <= 10 THEN 1
        WHEN c.AUMENTO_PERCENTUAL <= 20 THEN 2
        ELSE 3
    END AS ORDEM_RISCO,
