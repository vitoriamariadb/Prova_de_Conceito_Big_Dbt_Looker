...
| DESCRICAO_CLUSTER | STRING | Alto Desempenho, Médio, etc |
| NOTA_MEDIA_ENEM | FLOAT64 | Média ENEM |
| RENDA_MEDIA_FAMILIAR | FLOAT64 | Renda média |
| PCT_ESCOLAS_INTERNET | FLOAT64 | % internet |
...
| IMPACTO_NOTA_ENEM_PONTOS | FLOAT64 | Impacto esperado na nota ENEM |
| REDUCAO_ABANDONO_PCT | FLOAT64 | Redução esperada no abandono |
| IMPACTO_IDEB_PONTOS | FLOAT64 | Impacto esperado no IDEB |
| AVALIACAO_RISCO | STRING | Avaliação do risco fiscal |
| DATA_PROCESSAMENTO | TIMESTAMP | Data de processamento |
